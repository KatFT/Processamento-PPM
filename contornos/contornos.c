...
void execute(FILE* f1, FILE* f2){
  ...
  //vai por a primeira e ultima linha/coluan a zero
    for(int i=0;i<image.height;i++){
      for(int j=0;j<image.width;j++){
          image1.mat[0][j].r=0;
          image1.mat[0][j].g=0;
          ...
          image1.mat[image.height-1][j].r=0;
          image1.mat[image.height-1][j].g=0;
          image1.mat[image.height-1][j].b=0;

          image1.mat[i][0].r=0;
          image1.mat[i][0].g=0;
          image1.mat[i][0].b=0;

          image1.mat[i][image.width-1].r=0;
          image1.mat[i][image.width-1].g=0;
          image1.mat[i][image.width-1].b=0;
  ...
  for(int i=1;i<image.height && (i+1)<image.height && (i-1)>=0;i++){
      for(int j=1;j<image.width && (j+1)<image.width &&(j-1)>=0;j++){
            ...
            p.r=(8*image.mat[i][j].r) -(image.mat[i-1][j-1].r+image.mat[i-1][j].r+image.mat[i-1][j+1].r+
            image.mat[i][j-1].r + image.mat[i][j+1].r + image.mat[i+1][j-1].r+image.mat[i+1][j].r+image.mat[i+1][j+1].r);
            image1.mat[i][j].r=max(0,min(p.r,image.maxval));
            image1.mat[i][j].g=max(0,min(p.r,image.maxval));
            image1.mat[i][j].b=max(0,min(p.r,image.maxval));         
      }     
  }
